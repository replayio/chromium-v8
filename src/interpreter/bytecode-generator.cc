...
void BytecodeGenerator::ReplayExpressionShiftedSetStatementPosition(Statement* stmt, Expression* expr) {
  recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: stmt->position(): %d %d\n", script_.is_null() ? -1 : script_->id(), stmt->position());
  recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: expr->position(): %d %d\n", script_.is_null() ? -1 : script_->id(), expr->position());
  ...
  if (expr->position() < 0) {
    ...
    recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: add at stmt position: %d %d\n", script_.is_null() ? -1 : script_->id(), stmt->position());
    ...
    recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: add at left operand: %d %d\n", script_.is_null() ? -1 : script_->id(), expr->AsBinaryOperation()->left()->position());
    builder()->RecordReplayInstrumentation("breakpoint", expr->AsBinaryOperation()->left()->position());
  } else {
    recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: add at expression: %d %d\n", script_.is_null() ? -1 : script_->id(), expr->position());
...
void BytecodeGenerator::VisitCall(Call* expr) {
  ...
  recordreplay::Print("VisitCall: call_head_token_position: %d %d\n", script_.is_null() ? -1 : script_->id(), expr->call_head_token_position());
  recordreplay::Print("VisitCall: start_locations_size: %d %d\n", script_.is_null() ? -1 : script_->id(), start_locations_size);
  recordreplay::Print("VisitCall: builder()->record_replay_instrumentation_site_locations_.size(): %d %d\n", script_.is_null() ? -1 : script_->id(), builder()->record_replay_instrumentation_site_locations_.size());
