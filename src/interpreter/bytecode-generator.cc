...
void BytecodeGenerator::ReplayExpressionShiftedSetStatementPosition(Statement* stmt, Expression* expr) {
  recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: stmt->position(): %s %d\n", script_->id(), stmt->position());
  recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: expr->position(): %s %d\n", script_->id(), expr->position());
  ...
  if (expr->position() < 0) {
    ...
    recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: add at stmt position: %s %d\n", script_->id(), stmt->position());
    ...
    recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: add at left operand: %s %d\n", script_->id(), expr->AsBinaryOperation()->left()->position());
    builder()->RecordReplayInstrumentation("breakpoint", expr->AsBinaryOperation()->left()->position());
  } else {
    recordreplay::Print("ReplayExpressionShiftedSetStatementPosition: add at expression: %s %d\n", script_->id(), expr->position());
...
void BytecodeGenerator::VisitCall(Call* expr) {
  ...
  recordreplay::Print("VisitCall: call_head_token_position: %s %d\n", script_->id(), expr->call_head_token_position());
  recordreplay::Print("VisitCall: start_locations_size: %s %d\n", script_->id(), start_locations_size);
  recordreplay::Print("VisitCall: builder()->record_replay_instrumentation_site_locations_.size(): %s %d\n", script_->id(), builder()->record_replay_instrumentation_site_locations_.size());
